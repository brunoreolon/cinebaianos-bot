...
def sincronizar_filmes_com_planilha():
    print("🔄 Sincronizando filmes com a planilha...\n")
    ...
    for filme in filmes_planilha:
        ...
        if detalhes:
            ...
            print(f"✅ {detalhes.title} ({detalhes.ano}) adicionado.")
            total_filmes += 1
        else:
            print(f"⚠️ Detalhes não encontrados: {detalhes.title} ({detalhes.ano})")
...
def sincronizar_votos_com_planilha():
    ...
    for voto in votos:
        ...
        if not filme_info:
            print(f"❌ Filme não encontrado para responsavel={nome_responsavel}, linha={id_linha}")
            continue

        id_filme, titulo_filme = filme_info
        registrar_voto(id_filme, id_responsavel, id_votante, valor_voto)
        print(f"🗳️ Voto registrado: {nome_votante} votou '{valor_voto}' no filme '{titulo_filme}' (Aba={aba}, Responsável={nome_responsavel}, linha {id_linha})")
        total_votos += 1
